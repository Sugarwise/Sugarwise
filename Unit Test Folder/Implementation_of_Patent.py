...
fsugar_dic={}; fsugar_dic["honey"]=set(["mushrooms","flavour","flavouring","flavourings","flavours","artificial","flavour^","fungus"]);fsugar_dic["agave"]=set([]);
fsugar_dic["molasses"]=set(["flavouring","salt"]);fsugar_dic["sugar"]=set(["milk","fat","nibs"]);fsugar_dic["syrup"]=set(["malittol","milk","oligofructose","maltitol","sorbitol","skimmed","scribitol",]);
fsugar_dic["juice"]=set(["chicken","celery","vera","iceberg","hibiscus","sauerkraut","lime","beef","flavouring","mushroom","cod","garlic","cabbage","tomato","onion","truffle","dill","coconut","cashew","carrot-",]);
fsugar_dic["nectar"]=set([]);fsugar_dic["jam"]=set([]);fsugar_dic["jelly"]=set(["bonestock","pork"]);
fsugar_dic["concentrate"]=set(["safflower","fennel","chicken","milk","celery","dairy","coffee","spirulin","purple","pepper","algae","chilli","soybean","soy","lime","beef","shiitake","parsley","butter","paprika",
"protein","carrot-juice","lamb","lemon","mushroom","cashew","vegetable","leek","garlic","carrot-","watercress","oyster","fermented","spirulina","nettle","turmeric","egg"]);
fsugar_dic["glucose"]=set(["maize","flavouring","oxidase"]);fsugar_dic["sucrose"]=set(["isobutyrate"]);fsugar_dic["maltose"]=set([]);fsugar_dic["dextrose"]=set([]);
fsugar_dic["fructose"]=set([]);fsugar_dic["sugars"]=set([]);fsugar_dic["syrups"]=set([]);fsugar_dic["juices"]=set(["pork:","mushroom","fish","citric","carrot","chicken","lime","lamb","lemon","beef","onion","meat","aronia"]);
fsugar_dic["palm"]=set(["nectar","treacle","sugar"]);
fsugar_dic["extract"]=set(["orange","mango","grape","malted","marnier","apple","liqueur","irish","beer","beet","blueberry","malt","honey","agave","hop","redcurrant","berry","fruit","cola","fig","cointreau","tiramisu","barley","bourbon","syrup","elderberry","blackcurrant"])
...
class Nutrient:
    ...
    def __init__(self,name,i):
        ...
        if name in ["maltose","glucose","sucrose","fructose","dextrose","lactose","sugar","protein","carbohydrate"]:
            self.calories=4
        elif name=="fat":
            self.calories=9
        else:
...
class Ingredient:
    ...
    def __init__(self,name):
        ...
        name_edit=name.replace(',',' ') #takes commas out of the ingredients names for better parsing, replaces them with double spaces
        name_edit=name_edit.replace('  ',' ') #replaces double spaces in ingredient names with single spaces, for better parsing
        ...
        for word in fsugar_dic.keys():            
            if word=="palm" and word in name_split and len(fsugar_dic[word].intersection(name_split))>0: 
                ...
                self.fsugar_boolean=True
                break
            elif word!="palm" and word!="extract" and word in name_split and len(fsugar_dic[word].intersection(name_split))==0:
                self.fsugar_boolean=True
                break
...
def parser(db_filename,ingredient_lst_filename,lab_filename):
    ...
    for i in range(1,len(lab_parsed[0])): #the loop starts at i=1 because the first word of the row is "Name" for the first row, and "Lab Amount" for the second row
        if lab_parsed[1][i]!='':  #skips blank values in the lab data csv file, which later on in the code signifies that that nutrient will not be considered in subsequent analysis
            nutrient=Nutrient(lab_parsed[0][i].lower(),i)  #makes a nutrient object with name and appropriate row index
            if i==4:  #i=4 here is the row index for carbohydrates, and i=5 is the row index for total sugars, WHICH IS HOW THE CSV FILES I SUPPLIED IS FORMATTED. A CHANGE IN FORMAT MUST BE ACCOMPANIED BY A CHANGE HERE
                lab_dic[nutrient]=float(lab_parsed[1][i])-float(lab_parsed[1][i+1]) #here, I am distinguinshing between sugars and carbohydrates by SUBTRACTING the amount of sugars from the carbohydrate amount 
        ...
        for nutrient in lab_dic.keys():
            try:   #this try and except clause is for cases when the database doesn't have a numerical amount for a certain nutrient, this catches the error, and the except clause sets the amount as 0
                if nutrient.row_index==4:
                    nutrient_amount=float(row[nutrient.row_index])-float(row[nutrient.row_index+1])   #same thing as before, subtracting the total sugar amount from the carbohydrate amount
                else:
                    nutrient_amount=float(row[nutrient.row_index])
                nutrient_dic[nutrient]=nutrient_amount 
            except ValueError:
                nutrient_dic[nutrient]=0  #so if there isn't a numerical amount for the nutrient in the databse, we assume it is zero
    ...
    product_lst=sorted(product_lst,key=lambda ingredient: ingredient.name) #reorders the ingredients by alphabetical order
...
def build_lin_system(product_lst,lab_dic):
    ...
    for ingredient in product_lst:
        for nutrient in ingredient.nutrient_dic:
            if nutrient.name not in lab_nutrient_lst:
                del ingredient.nutrient_dic[nutrient] #delets nutrients from the dic whose names are not among the nutrient's names in lab_dic
    ...
    for i in range(0,len(b)): #this loop gets the i_th nutrient from a list of sorted lab_dic keys, gets its amount and divides it by the normalizer
        nutrient_mass= lab_dic[sorted(lab_dic.keys(),key=lambda nutrient: nutrient.name)[i]] 
        b[i]=nutrient_mass/normalizer 
    A=np.zeros([len(lab_dic.keys()),len(product_lst)]) #initializes A according to patent: M is number of tested nutrients, N is number of ingredients
    ...
    for nutrient in lab_dic.keys():
        if nutrient.sugar_boolean and nutrient.name not in [ingredient.name for ingredient in product_lst]: #this checks if the nutrient is a sugar, and if it ISN'T already a ingredient
            ...
            new_ingredient.nutrient_dic[nutrient]=100 # and this makes the nutrient amount for the sugar ITSELF 100g per 100g
    ...
    for i in range(0,A.shape[0]):
        for j in range(0,A.shape[1]): 
            ...
            nutrient_i=sorted(ingredient_j.nutrient_dic,key=lambda nutrient: nutrient.name)[i] #this makes sure that indexes are distributed to the ingredient according to alphabetical order
            A[i,j]=ingredient_j.nutrient_dic[nutrient_i]/normalizer  #calculates the matrix elements as defined in the patent
    ...
    for i in range(0,A_prime.shape[1]): #looping through indices will make the order of the coefficients the same as the order of the ingredients in the matrix
        fsugar_amt=0        
        for nutrient in product_lst_prime[i].nutrient_dic: #this loop will just picks out the amount of total sugar in the ingredient and adds it to the fsugar functional coefficients
    ...
    for i in range(0,A_prime.shape[1]):
        ...
        for nutrient in product_lst_prime[i].nutrient_dic:
            calories_counter=calories_counter+(nutrient.calories*(product_lst_prime[i].nutrient_dic[nutrient]/normalizer))
...
def get_recipe(A,b,A_prime,fsugar_functional_numerator_coeff,fsugar_functional_denominator_coeff,tol):
    ...
    if A_rank==A_b_rank and A_rank==A.shape[1]:
        ...
        solution=np.linalg.lstsq(A,b)[0].transpose()[0] #we solve the system by least squares because, that the if statement is true, it is GUARANTEED that the least squares solution is the real solution, and this algorithim does not rely on matrix inversion(for which a square matrix is needed)
    ...
    if (A_rank==A_b_rank and A_rank<A.shape[1]) or A_b_rank>A_rank or solution=="overdetermined":
        A_prime_eq=np.concatenate((A_prime,-b),1)  #given that, because of the transformation, Ax=b-->Ay=bt-->Ay-bt=0, we can write this as (A|-b)y_t=0
        extra_fractional_constraint=np.array(fsugar_functional_denominator_coeff+[0]) #we add zero to account for the fact that, in the transformed linear program, the coefficient for the t variable is 0 in the objective function's denominator
        A_prime_eq=np.concatenate((A_prime_eq,[extra_fractional_constraint]),0) # we concatenate the denominator constraint onto A_prime_eq because linprog takes in as input a matrix with all equality constraints
        b_eq=np.zeros([1,A_prime.shape[0]+1])[0]; b_eq[-1]=1 #this vector contains the zeros for all the equations expressed in the constraint Ay-bt=0, AND the 1 for the extra_fractiononal_constraint=1   
        A_prime_ub=np.array([[1]*A_prime.shape[1]+[-1]]) #this includes the coefficients to the express the constraint: sum(x)<1 --> (sum(y))-t<0  
        ...
        linprog_result=op.linprog(minimizing_function, A_eq=A_prime_eq, b_eq=b_eq, A_ub=A_prime_ub, b_ub=b_ub,bounds=(0, 1))  #bounds no less than zero because we are looking for positive quantities
    ...
    #using the SOCP solver algorithim by CVXOPT. This requires making special matrices and vectors to put the problem into canonical form for the solver. 
    ...
    if solution=="overdetermined":
        ...
        G0_q=np.array([[0]*A_prime.shape[1]+[-tol]])  #this is the c.Transpose row vector, which for this problem, is [00...tol], since only the variable t has a coefficient 
        G1_q=-np.concatenate((A_prime,-b),1) #Note that tol and (A|-b) have a negative sign in front of it, according to the thoery
        G_normq=np.concatenate((G0_q,G1_q),0)
        G_normq=cvx.matrix(G_normq.astype(np.double)) #the number type is cast as double because that is what CVXOPT accepts       
        ...
        for i in range (G1_q.shape[1]): #this loop concatenates [00...1...0] row vectors, which signify the positivity constraints. G1_q.shape[1] is just A_prime.shape[1]+1
            ineq=[0]*G1_q.shape[1]
            ineq[i]=-1        
        ...
        b=cvx.matrix([1.0])
        minimizing_function_cvx=-cvx.matrix(fsugar_functional_numerator_coeff+[0.0]) # CVXOPT only does minimization, so we must use the negative of the numerator of the objective function        
...
def sugarwise_test(db_filename,ingredient_lst_filename,lab_filename,tol):
    ...
    if solution!= "infeasible":   
        ...
        for i in range(len(solution)): #this loop calculates the fraction of calories derived from free sugars from the calculated solution vector
